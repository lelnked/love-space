...
### TC-merchant-WEB-003: 商户表单名称 16~1000 字可保存回显
**关联需求**: merchant/商户名称长度上限#web 表单同口径校验
**来源**: merchant-name-length-1000
**优先级**: P1
**前置条件**: Manager 已登录；存在可编辑商户
**测试步骤**:
1. 打开商户编辑表单，名称改为 23 字门店名（如「深圳南山区科技园万象天地店」）并保存
2. 重新打开该商户编辑表单，检查名称回显
3. 名称改为 1001 字文本后点击保存
**预期结果**: 步骤 1 保存成功（不再出现「名称最多 15 个字符」）；步骤 2 名称原样回显；步骤 3 表单按既有校验口径提示「名称最多 1000 个字符」，不发起保存
**状态**: ⬜ 未测试
**执行方式**: 
**执行存证**: 
**最后更新**: 2026-09-11
