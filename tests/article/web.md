...
**状态**: ✅ 通过
**执行方式**: web-test-runner（@playwright/mcp）
**执行存证**: `test-evidence/regression/article/TC-article-WEB-002/`
**阻塞说明**: 无阻塞。已验证文章列表展示图片、标题、关联栏目、权重、状态开关与操作列；切换状态开关后状态即时更新且出现成功提示。
**最后更新**: 2026-08-21
...
**状态**: ✅ 通过
**执行方式**: web-test-runner（@playwright/mcp）
**执行存证**: `test-evidence/regression/article/TC-article-WEB-003/`
**阻塞说明**: 无阻塞。已验证文章表单保存成功并跳转列表页；重新打开编辑表单后富文本编辑器回显此前录入的文本与图片；栏目多选回显为之前勾选的两个栏目。
**最后更新**: 2026-08-21
