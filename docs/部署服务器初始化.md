...
# 只传 *.sh 与模板。.env.prod 在服务器上创建（见下一步），不要在本地留生产密钥。
scp deploy/.env.example                             root@119.29.108.66:/app/loveSpace/deploy/
```

### 3. 配置环境变量（只需一次）

**服务器上环境变量的唯一来源是 `/app/loveSpace/deploy/.env.<环境>`**（生产 `.env.prod`，
测试 `.env.test`），不写进 `~/.bashrc`。**这份文件属于服务器，在服务器上创建和维护，
打包机不留副本**：
...
需要填的项：DB 地址、`ADMIN_JWT_SECRET`、`APP_SECURITY_API_KEYS`、
阿里云 OSS 六项（`ALIYUN_OSS_*` / `ALIYUN_STS_ROLE_ARN`）。
密钥值以团队密钥管理为准，不写进本文档；文件被 `.gitignore` 忽略，也不会误入库。
