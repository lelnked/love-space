...
> 本文档只覆盖应用部署流程，不包含 PostgreSQL 初始化（PostgreSQL 部署见 `DEPLOY.md` 第四节）。
...
bash deploy-apps.sh 1.0.0    # 版本号自定义，会作为 docker 镜像 tag
```
...
- **只更后端**：本地 `./mvnw clean package -DskipTests` → `scp` jar → 服务器 `bash deploy-apps.sh 1.0.x`（版本号递增）
