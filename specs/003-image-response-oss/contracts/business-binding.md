...
- 同一 `objectKey` 第二次 `validateAndBind` 会发现 OSS 上 `images/<uuid>.<ext>` 已不存在（首次绑定时已 delete），从而抛 `ValidationException` —— 因此调用方 MUST 在每次需要新图时重新走"直传 → 提交"流程，不要复用旧 objectKey。
...
String boundKey = objectKeyValidator.validateAndBind(rawObjectKey);
// 内部：
//   - rawObjectKey 匹配 images/ → head + 校验 + copy(images→bound) + delete(images) → 返回 bound key
