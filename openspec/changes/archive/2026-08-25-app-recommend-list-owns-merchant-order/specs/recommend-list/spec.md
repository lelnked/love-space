...
**Reason**: 该需求把"清单内商户"同时挂在两个接口上——`GET /api/app/recommend-lists/{id}` 与 `GET /api/app/merchants/page?recommendListId=`，后者让商户域反向依赖推荐清单域（join 关联表、在商户排序之上硬加清单排序、给商户项塞 `recommendSortOrder`），且把关系表的位置列 `sort_order` 暴露成对外业务概念。scenario「按推荐清单过滤商户列表」随之废止。新需求「App 端清单与清单内商户查询」承接其余三个 scenario 并收敛清单商户项字段。
**Migration**: 移动端清单页改为消费 `GET /api/app/recommend-lists/{id}` 的 `merchants[]`（字段 `id/name/address/logo`，`merchantId` 改名 `id`，`recommendReason`/`sortOrder` 不再返回，顺序即数组顺序）。`GET /api/app/merchants/page` 若仍携带 `recommendListId`，参数被忽略、按普通列表返回，不报错。无数据迁移。
