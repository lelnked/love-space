...
## Purpose
图片上传凭证与对象存储：STS 直传签名、objectKey 两段式生命周期（images/ → bound/）与绑定校验契约、限时签名访问地址，以及该链路的自动化覆盖边界。
## Requirements
...
### Requirement: 图片签名访问地址
读取带图业务数据时，服务端 SHALL 将存储的 objectKey 装配为 `{id, url}` 结构下发——`id` 为 objectKey，`url` 为即时生成的限时签名访问地址，默认有效期 1800 秒。

签名地址 SHALL 每次请求即时生成，不做缓存。objectKey 为空时 SHALL 返回空值而不生成地址。列表形式的图片字段 SHALL 保持原有顺序并跳过空值项。

admin 与 app 两端的签名语义 SHALL 保持一致（相同的默认有效期与生成方式）；app 端仅需读取权限，不提供任何上传端点。

#### Scenario: 业务详情返回签名地址
- **GIVEN** 一条已保存的带图业务数据
- **WHEN** 查询其详情
- **THEN** 图片字段为 `{id, url}` 结构，`id` 为 `bound/` 前缀的 objectKey，`url` 为带签名参数的访问地址

#### Scenario: 空图片字段不生成地址
- **GIVEN** 一条图片字段为空的业务数据（如未设置背景图的城市）
- **WHEN** 查询其详情
- **THEN** 该图片字段为 null，不返回地址

#### Scenario: 多图字段保持顺序
- **GIVEN** 一条含 3 张图片的业务数据
- **WHEN** 查询其详情
- **THEN** 图片数组按保存时的顺序返回
...
### Requirement: 图片链路的自动化覆盖边界
本域的对象存储交互 SHALL 在测试环境以桩实现替代：测试档位下绑定校验只保留格式校验并直接返回 `bound/` 前缀结果，不进行任何存储访问。

因此以下行为**无法由自动化测试覆盖**，SHALL 以人工或联调方式验证：浏览器到对象存储的真实直传、临时凭证的真实签发、已绑定对象的真实可读性。

`POST /api/admin/files/upload-credentials` 的成功分支在测试档位下**不可实跑**（临时凭证签发需真实网络与真实角色配置），其接口测试仅覆盖鉴权与入参校验分支。

绑定校验的存在性、类型、大小三项校验，以及签名地址生成，SHALL 由单元测试以模拟存储客户端覆盖。

#### Scenario: 测试档位下绑定校验不访问存储
- **GIVEN** 服务运行在测试档位
- **WHEN** 提交任一带图业务写请求，图片为格式合法但存储中不存在的 objectKey
- **THEN** 请求成功，objectKey 被改写为 `bound/` 前缀——不因对象不存在而失败

#### Scenario: 上传凭证成功分支在测试档位不可用
- **GIVEN** 服务运行在测试档位
- **WHEN** 以合法 contentType 请求上传凭证
- **THEN** 因临时凭证签发不可用而无法返回 200，该分支不纳入自动化断言
