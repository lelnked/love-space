# TC-merchant-IT-007 请求/响应存证

用例: GET /api/app/categories/page 同排序号分类按创建时间倒序
...
## Step 2: 先创建分类 A（sortOrder=0，上架）

```bash
curl -s -X POST "http://localhost:21423/api/admin/categories" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"name":"分A021653","sortOrder":0,"online":true}'
```

实际响应（HTTP/1.1 200）:

```json
{"id":"01a03bdb-cdc5-7654-aee9-faa1c0bc5502","name":"分A021653","sortOrder":0,"online":true,"createdAt":"2026-08-26T02:17:35.940297286Z"}
```

## Step 3: 后创建分类 B（sortOrder=0，上架）

```bash
curl -s -X POST "http://localhost:21423/api/admin/categories" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"name":"分B021653","sortOrder":0,"online":true}'
```

实际响应（HTTP/1.1 200）:

```json
{"id":"01a03bdb-cdd2-76aa-bcbb-ea8bad0cae63","name":"分B021653","sortOrder":0,"online":true,"createdAt":"2026-08-26T02:17:35.954377836Z"}
```

## Step 4: 连库拉开 created_at（A 前移 2 小时）

```bash
psql "postgresql://iris:$PGPASSWORD@localhost:25432/love_space" -c "update loves_category set created_at = created_at - interval '2 hour' where id='01a03bdb-cdc5-7654-aee9-faa1c0bc5502';"
```
...
```json
UPDATE 1
分B021653|0|2026-08-26 02:17:35.954378+00|t
分A021653|0|2026-08-26 00:17:35.940297+00|t
```
