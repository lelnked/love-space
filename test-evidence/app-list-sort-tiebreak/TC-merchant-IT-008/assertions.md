...
| 3 | body 为分页对象 | ✅ | `['content', 'page', 'size', 'totalElements', 'totalPages']` |
| 4 | B（sortOrder=0，后创建）排在 A（sortOrder=1，先创建）之前 | ✅ | `idx(序B)=0 < idx(序A)=15` |
| 5 | 排序号为主序：sortOrder=0 段整体在 sortOrder=1 段之前 | ✅ | `序B 位列第 1；序A 位于全部 sortOrder=0 分类之后` |
