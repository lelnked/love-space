...
- ✅ period 为 JSON 数组而非字符串 — ['MENSTRUAL']
- ✅ 条目不含 activityId/routeId/articleId — ['banner', 'description', 'id', 'note', 'period', 'subtitle', 'targetId', 'title', 'type']
- ✅ Step 2: 每条 period 为 ["MENSTRUAL"]（三个 target 各自只配在一个周期） — [['MENSTRUAL'], ['MENSTRUAL']]
- ✅ Step 2: 不含 FOLLICULAR 条目 — 2
- ✅ 返回 200 — 200
- ✅ Content-Type 含 application/json — application/json
- ✅ 响应顶层为 JSON 数组 — list
- ✅ 数组恰含 1 条 — 1
- ✅ period 为 JSON 数组而非字符串 — ['FOLLICULAR']
- ✅ 条目不含 activityId/routeId/articleId — ['banner', 'description', 'id', 'note', 'period', 'subtitle', 'targetId', 'title', 'type']
- ✅ Step 3: 恰含 1 条且 period 为 ["FOLLICULAR"] — ['FOLLICULAR']
